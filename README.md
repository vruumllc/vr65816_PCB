...
4. Unfortunately, the 6502 memory locations reserved for the 'RIA registers', used to communicate between the Pico 2W REA and the 6502, overlap the 65816 16-bit interrupt vectors.  I'm hoping we can switch into 8-bit emulation mode before throwing an interrupt. As a very last resort, I added the mysterious jumper JP4, which can increase the space for the REA registers enough to shift them down. This means #defining all the register locations in the firmware and compilers, and breaking compatibility with 6502 ROMs, which I really, really, really don't want to do.
...
8. The vr65816 PCB is 25mm taller than the RP6502 PCB, so it won't fit in my fabulous 3D-printed cases for it. If I ever finish working on this README and the vr65816_examples, I'll hop right to it and come up with something new.
