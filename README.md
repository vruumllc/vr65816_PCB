...
Should you be building the vr65816 at all? This board is targeted at experienced programmers interested in exploring and exploiting the 65816 microprocessor. You might want to read the Caveat section at the end of this document before continuing. I built it because I wanted to create projects requiring more memory than the 64kB limit of the 8-bit 6502 processor, and I'm willing to experiment, and fail. 
...
All of this is a wonderful base for my project, but I went into it knowing that since it was developed for the 6502, that there would some limitations re-using it. I also knew I had NO intentions of forking any of his architecture, and instead would learn to work within the limitations. You should too. 

Here are some things to keep in mind before building my vr65816:

1. Rumbledethump has stated that he has no interest in supporting the 65816. That means this project is a kind of a, umm, love child [;^/} of his efforts. Please don't ask him to add any new features in support of this project.
...
3. Very importantly, neither C compiler supporting the RP6502 compiles for the 65816. The ca65 assembler piece of the [cc65 project](https://cc65.github.io/) DOES understand 65816 assembly, though. (I'm not sure about [llvm-mos's](https://llvm-mos.org/wiki/Welcome)  assembler.) This means that to exploit the advanced features and memory of the vr65816, you will need to code in assembly language. See my examples in the [vr65816_examples](https://github.com/vruumllc/vr65816_examples) repository.
4. Unfortunately, the 6502 memory locations reserved for the 'RIA registers', used to communicate between the Pico 2W RIA and the 6502, overlap the 65816 16-bit interrupt vectors.  I'm hoping we can switch into 8-bit emulation mode before throwing an interrupt. As a very last resort, I added the mysterious jumper JP4, which can increase the space for the RIA registers enough to shift them down. This means #defining all the register locations in the firmware and compilers, and breaking compatibility with 6502 ROMs, which I really, really, really don't want to do.
5. The RIA can only access the lowest 64kB of memory, so each 65816 ROM will have to be responsible for loading assets and code into higher memory. Large 65816 projects will need to be designed so that the initially loaded portion is less than 64kB.
